...
class Determination_5000 extends Simulation { 

  val records = jsonFile("determination_5000.json").readRecords
  ...
  val NumberOfItems = records(0).get("Items").map(_.toString).getOrElse("0").toInt
  ...
  val scn3 = scenario("50000 Req/Min Quote to SAP Tax calculator engine")
    .during(DurationMinutes minutes) {
      exec(http("exec_rpm").post(EndpointDet).body(StringBody(request)))
    }
